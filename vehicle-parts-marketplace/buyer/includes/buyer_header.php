...
$cart_count = 0;
$unread_ticket_replies = 0; // ✅ Support replies not seen
...
// ✅ Fetch unread replies from support
if ($logged_in) {
    try {
        $stmt = $pdo->prepare("
            SELECT COUNT(*) 
        ...
        ");
        $stmt->execute([$_SESSION['user_id']]);
        $unread_ticket_replies = (int)$stmt->fetchColumn();
    } catch (Exception $e) {
        error_log("Unread replies count failed: " . $e->getMessage());
        $unread_ticket_replies = 0;
    }
}
...
if ($logged_in) {
    try {
        $stmt = $pdo->prepare("SELECT COUNT(*) FROM cart_items WHERE buyer_id = ?");
        $stmt->execute([$_SESSION['user_id']]);
...
if (!isset($pdo)) {
    try {
        ...
        error_log("Failed to get PDO in buyer header: " . $e->getMessage());
    }
}
    ...
    <div class="p-6 border-b border-gray-700 bg-gradient-to-b from-blue-900 to-sidebar-bg">
      <div class="flex items-center space-x-3">
        ...
        <div>
          ...
          <p class="text-blue-200 text-sm">Buyer Panel</p>
        </div>
      </div>
    </div>

    <!-- Navigation - Now with scrolling -->
    <nav class="mt-4 px-2 h-[calc(100%-200px)] overflow-y-auto">
      ...
      <a href="wishlist.php" class="flex items-center px-6 py-3 text-gray-300 hover:bg-sidebar-hover hover:text-white rounded-lg transition <?= basename($_SERVER['PHP_SELF']) === 'wishlist.php' ? 'bg-sidebar-active text-white' : '' ?>">
        <i class="fas fa-heart mr-3"></i>
        <span>Wishlist</span>
      </a>
      <a href="cart.php" class="flex items-center px-6 py-3 text-gray-300 hover:bg-sidebar-hover hover:text-white rounded-lg transition <?= basename($_SERVER['PHP_SELF']) === 'cart.php' ? 'bg-sidebar-active text-white' : '' ?>">
        <i class="fas fa-shopping-cart mr-3"></i>
        <span>My Cart</span>
        <?php if ($cart_count > 0): ?>
          <span class="ml-auto bg-red-500 text-white text-xs w-5 h-5 rounded-full flex items-center justify-center"><?= $cart_count ?></span>
        <?php endif; ?>
      </a>
      ...
      <a href="../../buyer/browse_parts.php" class="flex items-center px-6 py-3 text-gray-300 hover:bg-sidebar-hover hover:text-white rounded-lg transition <?= basename($_SERVER['PHP_SELF']) === 'browse_parts.php' ? 'bg-sidebar-active text-white' : '' ?>">
      ...
      <a href="my_tickets.php" class="flex items-center px-6 py-3 text-gray-300 hover:bg-sidebar-hover hover:text-white rounded-lg transition <?= basename($_SERVER['PHP_SELF']) === 'my_tickets.php' ? 'bg-sidebar-active text-white' : '' ?>">
        ...
        <?php if ($unread_ticket_replies > 0): ?>
          <span class="ml-auto bg-red-500 text-white text-xs w-5 h-5 rounded-full flex items-center justify-center"><?= $unread_ticket_replies ?></span>
        <?php endif; ?>
      </a>
    </nav>
    ...
    <div class="bg-gradient-to-r from-blue-600 to-blue-800 text-white p-6 rounded-xl shadow-lg mb-8">
      <div class="flex justify-between items-start">
        ...
        <div class="flex items-center space-x-4">
          <!-- Cart Icon -->
          <a href="cart.php" class="relative w-10 h-10 bg-white/20 backdrop-blur-sm border border-white/30 rounded-full flex items-center justify-center text-white hover:bg-white/30 transition">
            <i class="fas fa-shopping-cart"></i>
            <?php if ($cart_count > 0): ?>
              <span class="absolute -top-1 -right-1 bg-red-500 text-white text-xs w-5 h-5 rounded-full flex items-center justify-center"><?= $cart_count ?></span>
            <?php endif; ?>
          </a>
          ...
          <button class="relative w-10 h-10 bg-white/20 backdrop-blur-sm border border-white/30 rounded-full flex items-center justify-center text-white hover:bg-white/30 transition">
            ...
            <?php if ($unread_ticket_replies > 0): ?>
              <span class="absolute -top-1 -right-1 bg-red-500 text-white text-xs w-5 h-5 rounded-full flex items-center justify-center"><?= $unread_ticket_replies ?></span>
            <?php endif; ?>
          </button>
